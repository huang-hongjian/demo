...
void RS_fec::matrix_mulGF256(PBYTE *a, PBYTE *b, PBYTE *c, int fec_num, int data_num, int data_len) {
    int i, j, k;
    //printf("m:%d,k:%d,s:%d\n",fec_num,data_num,data_len);
    for (i = 0; i < fec_num; ++i)
...
int RS_fec::fec_decode(PBYTE *data, PBYTE *fec_data, int lost_map[]) {
    ...
    for(int i = 0; i < lost_pkt_cnt; ++i) {
        ...
        //printf("cur_lost_pkt=%d [%p]\n",cur_lost_pkt,data[cur_lost_pkt]);
