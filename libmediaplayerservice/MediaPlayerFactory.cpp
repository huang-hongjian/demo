...
#include "FMediaPlayer/FMediaPlayerManager.h"
#include <cutils/properties.h>
namespace android {
...
class HiMediaPlayerFactory : public MediaPlayerFactory::IFactory {
  public:
    virtual float scoreFactory(const sp<IMediaPlayer>& client,
        ...
        property_get("service.media.cmccplayer", value, "");
...
class CMCCPlayerFactory : public MediaPlayerFactory::IFactory {
  public:
    virtual float scoreFactory(const sp<IMediaPlayer>& client,
        ...
        ALOGE("CMCCPlayerFactory::scoreFactory url=%s", url);
        ALOGE("CMCCPlayerFactory::scoreFactory curScore=%.2f, kOurScore=%.2f", curScore, kOurScore);
        if (kOurScore <= curScore)
            return 0.0;
    ...
    virtual sp<MediaPlayerBase> createPlayer() {
        ALOGE("Create CMCCPlayer");
        return new CMCCMediaPlayerManager();
    }
};

class FPlayerFactory : public MediaPlayerFactory::IFactory {
  public:
      virtual float scoreFactory(const sp<IMediaPlayer>& client,
                               const char* url,
                               float curScore){
        ...
        ALOGE("FPlayerFactory::scoreFactory url=%s", url);
        ALOGE("FPlayerFactory::scoreFactory curScore=%.2f, kOurScore=%.2f", curScore, kOurScore);
        char player_type[20];
        if(__system_property_get("runtime.iptv.player_type", player_type) == 0)
        {
        	ALOGE("runtime.iptv.stream null");
        	return 0.0;
        }
        if(strcmp(player_type,"fplayer") == 0)
             return 2.0;
        else 
            return 0.0;
    ...
    virtual sp<MediaPlayerBase> createPlayer() {
        ALOGE("Create FPlayer");
        return new FMediaPlayerManager();
    }
};
...
#endif
    registerFactory_l(new FPlayerFactory(), F_PLAYER);
    sInitComplete = true;
}
